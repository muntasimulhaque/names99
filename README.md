...
1. Open the project in Android Studio (Meerkat or newer). It will download Gradle 8.11.1 and all dependencies on first sync — accept any suggested Gradle/AGP updates if prompted.
...
The Gradle wrapper JAR is intentionally not committed; Android Studio regenerates it. From the command line, run `gradle wrapper` once (with any local Gradle ≥ 8.10) to restore `gradlew`.
